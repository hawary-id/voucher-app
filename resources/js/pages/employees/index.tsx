...
export default function EmployeeIndex({ employees: paginatedEmployees, filters }: Props) {
        ...
        <>
            ...
            <div className="container mx-auto space-y-6 p-6 max-w-7xl">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Kelola data induk karyawan, penempatan departemen, jabatan, dan status keaktifan.
                        </p>
                    </div>
                ...
                <div className="flex items-center gap-2 max-w-md relative">
                    ...
                    <Input
                        placeholder="Cari karyawan berdasarkan nama, NIK, atau jabatan..."
                        value={search}
                        ...
                        <table className={`w-full text-sm transition-opacity duration-200 ${isSearching ? 'opacity-50' : 'opacity-100'}`}>
                            <thead>
                                <tr className="border-b bg-muted/20 text-[10px] font-bold tracking-wider text-muted-foreground uppercase">
                                    ...
                                    <th className="p-4 text-left font-semibold">Departemen</th>
                                    ...
                                    <tr>
                                        <td colSpan={6} className="p-8 text-center">
                                            <div className="flex flex-col items-center justify-center space-y-2">
                                                ...
                                                <p className="font-medium text-muted-foreground text-sm">Data karyawan tidak ditemukan</p>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-44">
                                                        <DropdownMenuItem asChild>
                                                            <Link href={employees.edit(employee.id).url} className="flex items-center cursor-pointer">
                                                                <Edit2 className="mr-2 h-3.5 w-3.5 text-muted-foreground" /> Edit rincian
                                                            </Link>
                                                        </DropdownMenuItem>
                                                        <DeleteConfirmDialog
                                                            title="Hapus Karyawan"
                                                            description={`Apakah Anda yakin ingin menghapus karyawan "${employee.name}"?`}
                                                            onConfirm={() =>
                                                            ...
                                                            trigger={
                                                                ...
                                                                >
                                                                    <Trash2 className="mr-2 h-3.5 w-3.5" /> Hapus karyawan
                                                                </DropdownMenuItem>
                                                            }
                                                        />
                                                    </DropdownMenuContent>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-t pt-4">
                    <p className="text-sm text-muted-foreground">
                        Menampilkan <span className="font-medium text-foreground">{paginatedEmployees.data.length}</span> dari{' '}
                        <span className="font-medium text-foreground">{paginatedEmployees.total}</span> karyawan
                    </p>

                    <div className="flex flex-wrap items-center justify-center gap-1.5 sm:justify-start">
                        {paginatedEmployees.links.map((link, index) => (
                            <Button
                                key={index}
                                size="sm"
...
EmployeeIndex.layout = (page: ReactElement) => (
    <AppLayout
        breadcrumbs={[
            {
                title: 'Karyawan',
