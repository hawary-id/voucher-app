...
import { Plus, Search, MoreHorizontal, Edit2, Trash2, Layers } from 'lucide-react';
...
import type { PaginatedResponse, Department } from '@/types';
...
export default function DepartmentIndex({ departments: paginatedDepartments, filters }: Props) {
        ...
        <>
            ...
            <div className="container mx-auto space-y-6 p-6 max-w-7xl">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Kelola struktur departemen kerja, kode otorisasi, dan status operasionalnya.
                        </p>
                    </div>
                ...
                <div className="flex items-center gap-2 max-w-md relative">
                    ...
                    <Input
                        placeholder="Cari departemen berdasarkan nama atau kode..."
                        value={search}
                        ...
                        <table className={`w-full text-sm transition-opacity duration-200 ${isSearching ? 'opacity-50' : 'opacity-100'}`}>
                            <thead>
                                <tr className="border-b bg-muted/20 text-[10px] font-bold tracking-wider text-muted-foreground uppercase">
                                    <th className="p-4 text-left font-semibold">Kode Departemen</th>
                                    <th className="p-4 text-left font-semibold">Nama Departemen</th>
                            ...
                            <tbody className="divide-y divide-border/60">
                                {paginatedDepartments.data.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="p-8 text-center">
                                            <div className="flex flex-col items-center justify-center space-y-2">
                                                <Layers className="h-8 w-8 text-muted-foreground/60" />
                                                <p className="font-medium text-muted-foreground text-sm">Departemen tidak ditemukan</p>
                                                <p className="text-xs text-muted-foreground/80">Coba sesuaikan kata kunci pencarian Anda.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    paginatedDepartments.data.map((department) => (
                                        <tr key={department.id} className="hover:bg-muted/10 transition-colors">
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end" className="w-44">
                                                        <DropdownMenuItem asChild>
                                                            <Link href={departments.edit(department.id).url} className="flex items-center cursor-pointer">
                                                                <Edit2 className="mr-2 h-3.5 w-3.5 text-muted-foreground" /> Edit rincian
                                                            </Link>
                                                        </DropdownMenuItem>
                                                        <DeleteConfirmDialog
                                                            title="Hapus Departemen"
                                                            description={`Apakah Anda yakin ingin menghapus departemen "${department.name}"?`}
                                                            onConfirm={() =>
                                                            ...
                                                            trigger={
                                                                ...
                                                                >
                                                                    <Trash2 className="mr-2 h-3.5 w-3.5" /> Hapus departemen
                                                                </DropdownMenuItem>
                                                            }
                                                        />
                                                    </DropdownMenuContent>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-t pt-4">
                    <p className="text-sm text-muted-foreground">
                        Menampilkan <span className="font-medium text-foreground">{paginatedDepartments.data.length}</span> dari{' '}
                        <span className="font-medium text-foreground">{paginatedDepartments.total}</span> departemen
                    </p>

                    <div className="flex flex-wrap items-center justify-center gap-1.5 sm:justify-start">
                        {paginatedDepartments.links.map((link, index) => (
                            <Button
                                key={index}
                                size="sm"
...
DepartmentIndex.layout = (page: ReactElement) => (
    <AppLayout
        breadcrumbs={[
            {
                title: 'Departemen',
